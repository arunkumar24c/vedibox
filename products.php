    ...
    <script type="text/javascript" src="order/list.js"></script>
                        ...
                        <style>
                            #top_section table td { font-size: 15px; line-height: 15px; padding: 10px; font-weight: bold; }
                                                            #top_section { background-color: #660066; z-index: 1; }
                                                                                        #top_section table td { color: #ffffff; }
                            
                            .pricelist_table thead tr td { text-align: center; font-weight: bold; padding: 10px; font-size: 15px; }
                                                            .pricelist_table thead tr { background-color: #660066 !important; }
                            ...
                            .grid_products h1.category_row { text-transform: uppercase; }
                                                            .pricelist_table tr.category_row { background-color: #660066 !important; }
                                                                                        .pricelist_table tr.category_row { color: #ffffff !important; }
                            
                                                            .pricelist_table tr.product_row td { color: #000000 !important; }
                            
                                                            .pricelist_table tr:nth-child(odd) { background-color: #ffe6ff; }
                                                                                        .pricelist_table tr:nth-child(even) { background-color: #ffe6ff; }
                                ...
                                .pricelist_table tr td { vertical-align: middle; padding: 5px; border: 2px solid #000000; font-size: 16px; font-weight: 600; }
                                                            ...
                                                            .strike { text-decoration-color: #ff0000 !important; text-decoration-thickness: 3px !important; }
    ...
    <form name="order_form" method="POST">
        <div class="row mx-0" style="font-size: 14px; line-height: 15px;">
            ...
            <div class="col-xl-6">
                ...
                <button class="btn btn-success btnwidth submit_button" type="button" onClick="Javascript:SendOtp(event);">Submit</button>
                </div>
            </div>   
        </div>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body mb-4">
                ...
                <form name="otp_form" class="digit-group text-center" data-group-name="digits" data-autosubmit="true" autocomplete="off">
                    <div class="row">
                        <div class="col-12">
                            <div class="pfnt fw-600 pb-4 text-center">We have sent an OTP on your mobile number <br> <span class="otp_receive_mobile_number" id="otp_mobile"></span> </div>
                            ...
                            <input type="number" oninput="validateSingleDigitInput('digit-1')" id="digit-1" name="digit-1" data-next="digit-2" >
                            <input type="number" oninput="validateSingleDigitInput('digit-2')"  id="digit-2" name="digit-2" data-next="digit-3" data-previous="digit-1" />
                            <input type="number" oninput="validateSingleDigitInput('digit-3')" id="digit-3" name="digit-3" data-next="digit-4" data-previous="digit-2" />
                            <input type="number" oninput="validateSingleDigitInput('digit-4')" id="digit-4" name="digit-4" data-previous="digit-3" />
                            <div class="text-center pt-4">
                                <!-- <a href="Javascript:VerifyOTPnumber();" class="fw-400 h-blue text-center">Verify OTP?</a> -->
                                <button class="btn btn-success btnwidth " type="button" onClick="Javascript:VerifyOTPnumber();">Verify Otp</button>
...
<script type="text/javascript">
 function validateSingleDigitInput(id){

 
const oneDigitInput = document.getElementById(id);

oneDigitInput.addEventListener('input', function (event) {
  const inputValue = event.target.value;

  // Remove any non-digit characters except the first digit (e.g., if someone pastes multiple digits)
  event.target.value = inputValue.replace(/\D/g, '').substr(0, 1);
});
    ...
    function VerifyOTPnumber() {
        ...
        if(jQuery('.digit-group').length > 0) {
            ...
            if(jQuery('.otp_receive_mobile_number').length > 0) {
               otp_receive_mobile_number = jQuery('.otp_receive_mobile_number').html();
               otp_receive_mobile_number = jQuery.trim(otp_receive_mobile_number);
                // otp_receive_mobile_number ="8098629606";
                ...
                if(typeof otp_receive_mobile_number != "undefined" && otp_receive_mobile_number != null && otp_receive_mobile_number != "") {
                    ...
                    if(typeof otp_number != "undefined" && otp_number != null && otp_number != "") {
                        ...
                        jQuery.ajax({
                            type: 'POST',
                            url: "verify_otp.php",
                            data: {otp:otp_number, mobile_number:otp_receive_mobile_number},
                            success: function(result){
                                console.log(result)
                                ...
                                if(ret.status==true){
                                    jQuery('.otp_action').before('<div class="alert alert-success mb-2"> <button type="button" class="close" data-dismiss="alert">&times;</button>'+ret.message+'</div>');
                                             var form_name = "order_form";
                                             SubmitOrder(form_name);
                                }
                                else{
                                    jQuery('.otp_action').before('<div class="alert alert-danger mb-2"> <button type="button" class="close" data-dismiss="alert">&times;</button>'+ret.message+'</div>');
